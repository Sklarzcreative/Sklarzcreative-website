...
INGESTING = "INGESTING"        # artwork drawn outside the pipeline, being checked
BUILDING = "BUILDING"          # deterministic VECTOR_BUILD lane; never a model call
BUILT = "BUILT"                # deterministic output, awaiting human check vs its source
...
TRANSITIONS = {
    ...
    BLOCKED_HOLD: {ROUTED},
    CONTEXT_READY: {PROMPT_READY, BUILDING, INGESTING, HUMAN_CONFIRMATION_REQUIRED},
    PROMPT_READY: {GENERATING, HUMAN_CONFIRMATION_REQUIRED},
    GENERATING: {CANDIDATE_READY, REJECTED},
    # Artwork drawn outside the pipeline carries no citation and no test, so it
    # joins the generated lane rather than the deterministic one: it lands at
    # CANDIDATE_READY, whose only exit is OA_REVIEW. There is deliberately no
    # path from here to BUILT or to PRODUCTION_READY_BASE_ART.
    INGESTING: {CANDIDATE_READY, REJECTED, HUMAN_CONFIRMATION_REQUIRED},
    BUILDING: {BUILT, REJECTED, HUMAN_CONFIRMATION_REQUIRED},
    # A BUILT artifact needs no image review - there is no generated art to
    # second-guess - but a GENERATED candidate must always pass OA_REVIEW, so
    # CANDIDATE_READY deliberately has no path to PRODUCTION_READY_BASE_ART.
    BUILT: {PRODUCTION_READY_BASE_ART, HUMAN_CONFIRMATION_REQUIRED, REJECTED},
