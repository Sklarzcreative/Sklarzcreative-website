...
### Finding 8 — `getActiveSpreadsheet()` is null in a web app · **BLOCKER**

**This is why v1 could not be deployed at all.**
...
A web app has no open container, so there is no *active* spreadsheet.
`getActiveSpreadsheet()` returns `null`, and `.getSheets()` on null throws.

The failure mode is the dangerous part. v1's `doPost` wraps everything in
`try/catch` and returns `{ok:false}` — and the front end posts with
`mode: 'opaque'`, which means the browser **cannot read the response**. Every
visitor would have submitted their details, watched the Scorecard open exactly
as designed, and had nothing saved. **Silent, total data loss behind a
success-looking UI**, with no error anywhere the author would see.

**Fixed in v2.** `SpreadsheetApp.openById()` with the ID in Script Properties,
plus explicit tab names. Confirmed against Google's own documentation and
issue tracker: `getActive`/`getActiveSpreadsheet` return the active document
only where one exists, which is not the case in `doGet`/`doPost`.

*Sources:* [Class SpreadsheetApp](https://developers.google.com/apps-script/reference/spreadsheet/spreadsheet-app) ·
[issuetracker #189851066](https://issuetracker.google.com/issues/189851066)
